...
var screenSwitcher = new bodySwitcher('screen-switcher', 'Website Version: ');

//add a new class option ('classname', 'label')
screenSwitcher.defineClass('flashversion', 'Flash (recommended)');
screenSwitcher.defineClass('textversion', 'Basic');
...
function bodySwitcher(divid, label)
{
	...
	this.select.onchange = function()
	{
		...
		if (switcher.string == " flashversion ") {
			attach_flash();
		}
...
/*
function switch_body_to(switch_style, force) {
	if (switcher) {
		//alert("i can see switcher..");
	}
		//switcher.body.className = switcher.initial + " " + switch_style;
